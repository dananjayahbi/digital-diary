...
import { Card, SkeletonStatsCard, SkeletonChartCard, Skeleton } from '@/components/ui';
...
const InsightsPage = () => {
  ...
  const { journalStreak, longestStreak, activeDays, isLoading: streaksLoading } = useStreaks();
  const { entries, isLoading: entriesLoading } = useDiary({ limit: 100 });
  const { tasks, isLoading: tasksLoading } = useTasks({});

  const isLoading = streaksLoading || entriesLoading || tasksLoading;
